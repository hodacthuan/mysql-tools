 ...
 * Copyright (C) 2006 Mikael Ronstrom & Dolphinics ICS ASA 
 * Copyright (c) 2011, 2015 Oracle and/or its affiliates
 *
 * Added option to use range of warehouses to generate
 * Added option to specify first warehouse to generate
 *
 * 14 mar 2011
 *
...
int main(int argc, char *argv[])
{
	...
	if (argc < 2) {
		printf("Usage: %s -w # [-m #] [-c #] [-i #] [-o #] [-s #] [-n #] [-d <str>]\n", argv[0]);
		printf("\n");
		printf("-w #\n");
		printf("\twarehouse cardinality\n");
		printf("-m #\n");
		printf("\tnumber of first warehouse\n");
		printf("-c #\n");
	...
	/* Parse command line arguments. */
        ...
        {
          if(!strcmp(argv[argvIndex],"-w"))
            warehouses= (++argvIndex < argc) ? atoi(argv[argvIndex]) : 0;
          else if(!strcmp(argv[argvIndex],"-m"))
            first_warehouse= (++argvIndex < argc) ? atoi(argv[argvIndex]) : 0;
          else if(!strcmp(argv[argvIndex],"-c"))
            customers= (++argvIndex < argc) ? atoi(argv[argvIndex]) : 0;
          else if(!strcmp(argv[argvIndex],"-i"))
            items= (++argvIndex < argc) ? atoi(argv[argvIndex]) : 0;
          else if(!strcmp(argv[argvIndex],"-n"))
            new_orders= (++argvIndex < argc) ? atoi(argv[argvIndex]) : 0;
          else if(!strcmp(argv[argvIndex],"-o"))
            orders= (++argvIndex < argc) ? atoi(argv[argvIndex]) : 0;
          else if(!strcmp(argv[argvIndex],"-d"))
            strcpy(output_path, (++argvIndex < argc) ? argv[argvIndex] : "");
          else if(!strcmp(argv[argvIndex],"--pgsql"))
            mode_string= MODE_PGSQL;
          else if(!strcmp(argv[argvIndex],"--sapdb"))
	...
	if (warehouses == 0) {
        ...
        if (first_warehouse == 0) {
                printf("-m must be at least 1\n");
                return 3;
        }
        last_warehouse= first_warehouse + warehouses;
