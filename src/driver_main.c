 ...
 *
 * Added option for warmup/cooldown period
 * Added option for only use warehouses with a certain spread
 * Added option for intermediate results timer
 *
 * 14 mar 2011
 *
...
int parse_arguments(int argc, char *argv[])
{
        ...
        for (i = 1; i < argc; i += 2) {
			...
			strcpy(dbt2_mysql_port, argv[i + 1]);
                        ...
                        warmup_time = atoi(argv[i + 1]);
                } else if (strcmp(flag, "cooldown") == 0) {
                        cooldown_time = atoi(argv[i + 1]);
                ...
                } else if (strcmp(flag, "intermediate_timer_resolution") == 0) {
                        intermediate_timer_resolution = atoi(argv[i + 1]);
