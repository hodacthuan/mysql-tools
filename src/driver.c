...
#ifdef WIN32
#define PORTABLE_FILE_HANDLE HANDLE
#else
#define PORTABLE_FILE_HANDLE int
#endif
...
PORTABLE_FILE_HANDLE log_mix;
PORTABLE_FILE_HANDLE intermediate_log;
...
int portable_create(PORTABLE_FILE_HANDLE *handle,
                    const char *file_name)
{
#ifdef WIN32
  DWORD create_flag = CREATE_ALWAYS;
  HANDLE loc_handle;

  loc_handle = CreateFile(file_name,
                GENERIC_READ | GENERIC_WRITE,
                FILE_SHARE_DELETE | FILE_SHARED_READ | FILE_SHARED_WRITE,
                NULL,
                create_flag,
                FILE_ATTRIBUTE_NORMAL,
                NULL);

  if (loc_handle != INVALID_HANDLE_VALUE)
  {
    *handle = loc_handle;
    return 0;
  }
...
#else
  int create_flags = O_RDWR | O_SYNC | O_CREAT;
  int ptr = open(file_name, create_flags, S_IWRITE | S_IREAD);
  if (ptr == (int)-1)
  {
    return errno;
  }
...
int portable_write(PORTABLE_FILE_HANDLE handle, char *buf, int len)
{
#ifdef WIN32
  if (WriteFile(handle, buf, len, &dummy, NULL))
  {
    return 0;
  }
  else
...
#else
  ssize_t ret_code;
  ret_code = write(handle, buf, len);
  if (ret_code == (ssize_t)0)
  {
    return 0;
  }
  else
...
int portable_close(PORTABLE_FILE_HANDLE handle)
{
#ifdef WIN32
...
void init_locks()
{
  ...
  for(i = 0; i < 3 ;++i)
  {
    ...
    {
      pthread_mutex_init(&mutex_terminal_state[i][j], 0);
    }
  }
  pthread_mutex_init(&mutex_mix_log, 0);
}
...
int init_driver_logging()
{
	...
	sprintf(log_filename, "%s%s", output_path, MIX_LOG_NAME);
        ret_code = portable_create(&log_mix, log_filename);
        if (ret_code)
        {
	...
	sprintf(intermediate_filename, "%s%s", output_path, INTERMEDIATE_FILENAME);
        ret_code = portable_create(&intermediate_log, intermediate_filename);
        if (ret_code)
        {
...
{
  return portable_write(intermediate_log,
                        buf,
                        len);
}
...
{
  portable_close(intermediate_log);
}
...
final_write_log_mix()
{
  ...
  {
    portable_write(log_mix,
                   mix_log_buffer_start_ptr,
                   mix_log_buffer_len);
  }
  portable_close(log_mix);
  return 0;
}
