...
#   2008, 2017 Oracle Oracle and/or its affiliaties. All rights reserved.
# ----------------------------------------------------------------------
#
...
unpack_tarballs()
{
  ...
  if test "x$PERFORM_BUILD_BENCH" = "xyes" ; then
    ...
    if test "x$PERFORM_BUILD_DBT2" = "xyes" ; then
      unpack_tarball ${DBT2_VERSION}
    fi
  fi
...
write_iclaustron_conf()
{
  ...
  write_conf "MYSQL_PATH=\"${MYSQL_BIN_INSTALL_DIR}/${MYSQL_VERSION}\""
  ...
  if test "x$PERFSCHEMA_FLAG" = "xyes" ; then
    if test "x$MYSQL_BASE" = "x5.6" || \
       test "x$MYSQL_BASE" = "x5.7" || \
       test "x$MYSQL_BASE" = "x5.8" ; then
...
write_ndbd_default()
{
  ...
  write_conf $CONFIG_LINE
  CONFIG_LINE="LongMessageBuffer=128M"
  write_conf $CONFIG_LINE
...
write_api_nodes()
{
  ...
  if test "x$USE_SHM" = "xyes" ; then
    ...
    ((SHM_KEY=500))
    ((MYSQL_NODE_ID=FIRST_MYSQL_SERVER_NODE_ID))
    for ((i_wan = 0; i_wan < NUM_MULTI_CONNECTIONS; i_wan++))
    do
      CONFIG_FILE="[SHM]"
      write_conf $CONFIG_FILE
      CONFIG_FILE="NodeId1: $NDBMTD_NODE_ID"
      write_conf $CONFIG_FILE
      CONFIG_FILE="NodeId2: $MYSQL_NODE_ID"
      write_conf $CONFIG_FILE
      CONFIG_FILE="ShmKey: $SHM_KEY"
      write_conf $CONFIG_FILE
      ((SHM_KEY+=1))
      ((MYSQL_NODE_ID+=1))
    done
...
{
  if test "x${TASKSET}" = "xtaskset" ; then
...
run_sysbench_step()
{
  ...
  do
    ...
    set_first_server_host
    set_first_server_port
    if test "x$SYSBENCH_INSTANCES" != "x1" ; then
...
check_parameters()
{
  if test "x$MYSQL_BASE" != "x5.1" && \
     test "x$MYSQL_BASE" != "x5.5" && \
     test "x$MYSQL_BASE" != "x5.6" && \
     test "x$MYSQL_BASE" != "x5.7" && \
     test "x$MYSQL_BASE" != "x5.8"; then
    ${ECHO} "Only support MYSQL_BASE equal to any of 5.1, 5.5, 5.6, 5.7 or 5.8"
    exit 1
  fi
...
NDB_SHM_SIZE="2M"
NDB_SHM_SPINTIME="0"
NDB_RECEIVE_BUFFER_MEMORY="2M"
NDB_TOTAL_SEND_BUFFER_MEMORY=
NDB_TCP_SEND_BUFFER_MEMORY=
NDB_SHM_SEND_BUFFER_MEMORY=
