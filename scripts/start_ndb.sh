  ...
  case $1 in
    ...
    --ndb_restart_initial_node )
      ...
      if test "x$PROCESS" = "x" ; then
        PROCESS="ndb_mgm"
        MGM_CMD="mgm_cmd"
        MGM_COMMAND="-e '${NODE_ID} RESTART -n -i -a -f'"
      else
    ...
    --ndb_restart_node )
      ...
      if test "x$PROCESS" = "x" ; then
        PROCESS="ndb_mgm"
        MGM_CMD="mgm_cmd"
        MGM_COMMAND="-e '${NODE_ID} RESTART -n -a -f'"
      else
...
if test "x$PROCESS" = "xmysql_install_db" ; then
  if test "x$MYSQL_BASE" != "x5.7" && \
     test "x$MYSQL_BASE" != "x5.8"; then
    if test -f "$MYSQL_PATH/bin/mysql_install_db" ; then
      MYSQL_INIT_ARGS="$MYSQL_PATH/bin/mysql_install_db --no-defaults"
    fi
    if test -f "$MYSQL_PATH/scripts/mysql_install_db" ; then
      MYSQL_INIT_ARGS="$MYSQL_PATH/scripts/mysql_install_db --no-defaults"
    fi
    ...
    if test -f "$MYSQL_PATH/bin/mysqld" ; then
      MYSQL_INIT_ARGS="$MYSQL_PATH/bin/mysqld --no-defaults --initialize-insecure"
    fi
    if test "x$MYSQL_INIT_ARGS" = "x" ; then
      echo "Found no mysqld in bin in $MYSQL_PATH"
      exit 1
    fi
  fi
...
if test "x${PROCESS}" = "xmysqld" ; then
  ...
  if test "x$NDB_ENABLED" = "xyes" ; then
    ...
    if test "x$MYSQL_BASE" = "x5.7" ; then
