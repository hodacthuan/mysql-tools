...
extended_usage()
{
  ...
  MacOSX/x86/gcc
  --------------
    C/C++ flags include -fno-common -arch x86_64.
    When 32-bits builds then x86_64 is replaced by i386
    ...
    Mac OS X builds will always be 64-bit by default, when --32 is added
    the build will be 32 bit instead. Thus the flag --m64 is added by default.
EOF
}
...
#    fi
#  fi
# Set for all benchmark runs to enable easier backtracing
...
{
  if test "x$fast_flag" != "xno" && test "x$compiler" = "xgcc" ; then
...
set_bsd_configs()
{
  ...
  elif test "x$fast_flag" != "xno" ; then
...
# by Luojia Chen at Sun.
  case "`uname -a`" in
    ...
    *5.10* | *5.11* | *5.12* )
      ;;
    *)
      usage "Only versions 8,9, 10, 11 and 12 supported for Solaris"
      exit 1
  esac
  if test "x$cpu_base_type" != "xx86" && \
  ...
  if test "x$compiler" = "xgcc" ; then
    set_cc_and_cxx_for_gcc
    if test "x$cpu_base_type" != "xx86" && test "x$cpu_base_type" != "xsparc" ; then
      usage "gcc currently not supported for Solaris on other than x86 and SPARC"
      exit 1
    fi
    if test "x$fast_flag" = "xyes" ; then
      LDFLAGS="$LDFLAGS -O3"
