...
edit_pfs_synch()
{
  if test "x$MYSQL_BASE" != "x5.1" ; then
    ...
    if test "x$MYSQL_BASE" = "x5.5" ; then
...
create_test_database()
{
  ...
  CTD_BASE_COMMAND="${CTD_BASE_COMMAND} --user=root"
...
drop_test_database()
{
  ...
  DTD_BASE_COMMAND="${DTD_BASE_COMMAND} --user=root"
...
setup_replication()
{
  ...
  CM_COMMAND="${CM_COMMAND} MASTER_USER=\"root\","
  CM_COMMAND="${CM_COMMAND} MASTER_PASSWORD=\"\","
