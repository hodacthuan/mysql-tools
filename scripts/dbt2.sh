...
set_dbt2_bench_taskset()
{
  ...
  if test "x$BENCH_TASKSET" = "xnumactl" ; then
    if test "$BENCHMARK_BIND" = "x" ; then
      ...
      LOC_TASKSET="$LOC_TASKSET $BENCHMARK_BIND"
    ...
    if test "x$BENCHMARK_BIND" != "x" ; then
      ...
      LOC_TASKSET="$LOC_TASKSET $BENCHMARK_CPUS"
    fi
  elif test "x$BENCH_TASKSET" = "xtaskset" ; then
    LOC_TASKSET="$BENCH_TASKSET $BENCHMARK_CPUS"
  fi
}
...
run_one_test()
{
  ...
  if [ $NUM_WAREHOUSES -ge $TOT_WARES ] ; then
    ...
    do
      ...
      ((EXTRA_WARMUP += 14))
      ((CONNECTIONS = LINE_NUM_WARES * LINE_TERMINALS))
      EXEC_COMMAND="$COMMAND --host ${MYSQLD_HOSTS[$i]}"
      EXEC_COMMAND="$EXEC_COMMAND --connections $CONNECTIONS"
